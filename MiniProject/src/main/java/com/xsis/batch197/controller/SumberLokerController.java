...
public class SumberLokerController extends BaseController {
	...
	private ModelAndView index(@PathVariable("bid") Long biodataId) {
		...
		XSumberLokerModel sumberLoker = this.lokerRepo.findByBiodataId(biodataId);
		// Passing biodata Id pada modal
		sumberLoker.setBiodataId(biodataId);
		view.addObject("sumberLoker", sumberLoker);
		return view;
	}

	@PostMapping(value = "/sumber-low-kerja/save")
	public ModelAndView save(@Valid @ModelAttribute("sumber-low-kerja") XSumberLokerModel sumberLoker,
			BindingResult result) {
		// menampilkan view dari folder sumberLoker file index.html
		ModelAndView view = new ModelAndView("sumber-low-kerja/index");
